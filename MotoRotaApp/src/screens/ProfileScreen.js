...
import { toursAPI, participationsAPI, favoritesAPI } from '../services/api';
...
export default function ProfileScreen({ navigation }) {
    ...
    const init = async () => {
        ...
        if (uname) {
            try {
                const [myRes, joinRes, favRes] = await Promise.all([
...
const styles = StyleSheet.create({
    ...
    statsRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: C.surface, borderRadius: 16, borderWidth: 1, borderColor: C.border, paddingVertical: 16, paddingHorizontal: 24, gap: 24 },
    statItem: { alignItems: 'center' },
    statNumber: { color: C.textPrimary, fontSize: 22, fontWeight: '800' },
